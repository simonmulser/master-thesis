...
\aju{Why even combine the L,F,T strategies? This is not explained and requires some justification - as the results show - it does not make sense in all cases since they require contradicting decisions in certain steps. So I would suggest  argue that combing some of them does not make sense and therefore was tested to see if the simulation really detects ''worst behaviour''}
...
The previously defined selfish mining scenarios are executed on an \textit{x86 Linux} host machine with 16 virtualised cores and 57.718 GB of memory, the same machine used to examine the deterministic behaviour of the simulation framework in chapter \ref{chap:evaluation}.
...
\end{figure}
\aju{Bigger figure, and maybe change the colors e.g., use dashed line for H}
