...
\todo[inline]{paragraph about relative gain/total gain}
...
\aju{Doesn't this also depend on the actual hashrate of the selfish miner? 
If the selfish miner has a large enough hash rate he can win the block race in more cases therefore earn more blocks relative to all other miners. Hence, he can earn a bigger share than other miners and economically dominate of the long run ... The sentence here reads like it is not a problem, maybe highlight that an attacker can actually benefit from this attack e.g., by improvements to the attack like double-spending and selfish-mining. 
Maybe (optional) also mention here briefly that such long lasting selfish mining has not been studied or observed yet and therefore its good to better understand those attacks to know why they do not happen or if they have just not been detected - this better justifies your work.}
...
The implemented framework allows an accurate and deterministic simulation of the blockchain by using directly the Bitcoin reference implementation and a realistic network topology.
...
Therefore a deterministic behaviour and a realistic reference scenario reflecting the Bitcoin network are defined.
