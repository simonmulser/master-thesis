The simulation framework provides all needed functionalities to orchestrate a peer-to-peer network where on each node is running the \textit{Bitcoin} reference implementation.
This peer-to-peer network is implemented as a \textit{Docker} network containing multiple \textit{Docker} containers each running one instance of \textit{Bitcoin-core}.
The framework also coordinates the block discovery in the network.
Based on a sequence defined in a configuration file the framework sends commands to the nodes which are then generating valid blocks.
This is possible because all nodes are executed in the so-called \textit{regtest mode}, where the CPU-heavy proof-of-work is disabled and the nodes are accepting and RPC-call from outside which lets them create immediately a new block.
After a simulation run, the framework parses the logs created by the nodes and based on them a report gets created which displays the key metrics of the simulation.
\todo[inline]{Go a little bit into the details what the rational behind this design was e.g., pre-defined sequence -> reproducability. Also outline the structure of this chapter a little might be helpful. First describe terms and concepts ... } 
  
\section{Tick}
A key concept of the simulation framework is a so-called tick.
...
This sequence is basically the simulation scenario for a particular simulation run.
The exact duration of a tick is defined on execution of the simulation, hence defining the speed of the simulation.
The smaller the defined duration, the higher the execution speed of the simulation.\todo{advantages: fake discrete, we can speed up/slow down}
\todo[inline]{I would call it "approximate discrete event behavior" or an abstraction of real time to simulation time. Also state the basic rule of a tick: During one tick a node is not allowed to mine more than one block.}

\section{Configuration files}

A simulation executed by the framework needs to be configured with the configuration files \textit{nodes.csv}, \textit{network.csv} and \textit{ticks.csv}.
The configuration files are stored in the concise CSV format and on a specific location on the disk to be easily processed by the simulation framework.
%This approach gives high flexibility and reproducibility for each simulation run.
\todo[inline]{Directly argue why this is flexible and note that the reproducibility is limited by the software under test or provide a reference to where this is explained}
The configuration files provide the flexibility to be written manually, to be created by a script or a combination of both.
Furthermore, the created configuration files can be copied to the output directory of a simulation run providing reproducibility.

The simulation framework already implements for each configuration file a script which can be executed with the corresponding commands \textit{nodes} (chapter \ref{chap:nodes_command}), \textit{network} (chapter \ref{chap:network_command}) and \textit{ticks} (chapter \ref{chap:ticks_command}).
\todo[inline]{Generally avoid using unspecific words and superlatives like: simple, highly something, ...} 
...
Each row in the file reflects one node containing:
\begin{itemize}
	\item \textit{node\_type}: Either \textit{bitcoin} if the node is a normal node or \textit{selfish} if the node should act as a selfish node.
	\item \textit{share}: The computational share of the node in the network.
	\item \textit{docker\_image}: The Docker image to use when starting the node.
...
The simulation framework starts each node in a way that a node on the y-axis tries to establish an outgoing connection to another node on the x-axis whenever the corresponding value in the matrix is set to 1.
...
Hence the \textit{ticks.csv} has no header describing the columns and the length of the lines varies, depending on the number of block events in the corresponding tick.

\section{Simulation} \label{chap:simulation}

The main functionality of the simulation framework is to orchestrate a simulation based on the configuration files.
This is achieved by using the high-level programming language \textit{Python} and the virtualization software technology \textit{Docker}.
\textit{Python} is mainly used to handle the configuration files and to automatically execute \textit{Docker} and other binaries whenever necessary.
...
\todo[inline]{The main benefit here is that the time returned by the system is synchronized among all containers, which mean the aggregation of the log is easy}
...
Subsequently, a virtual peer-to-peer network is build as depicted in figure \ref{fig:overview}.
...
Hence all the data persisted by \textit{bitcoind} under \textit{/data/regtest} will actually be persisted under \textit{data/run-10/node-5/} on the host machine and is therefore still available after the destruction of the container. \todo{julia}
...
Since this is needed to accept incoming connections, it is re-enable in line 9 by setting \textit{-listen} to 1.
...
\todo[inline]{This waiting might be a problem, it would be better to make these request asynchronous}
...
For the memory usage the thread reads periodically the \textit{MemAvailable} value in \textit{/proc/meminfo} file.
...
The simulation framework persists all parsed log lines into CSV files where each log line type gets his own file.
...
\begin{itemize}
	...
	\item charts and informations about blocks created during the simulation
	\item charts and informations about exceptions happened during the simulation
\end{itemize}
\todo[inline]{Maybe provide such a report in the appendix}

%Where most of the informations and charts are just simple representations of the data present in the CSV files, 
\todo[inline]{Do not undersell ;)}
The stale block rate and propagation time of blocks needs to be calculated in the report.
\todo[inline]{The term stale block rate should be explained in the State-of-the-Art section when talking about the other simulations - this is a key metric that is important and it should be clear why - maybe also cite Eyals Bitcoin NG}
...
After each simulation the created CSV files of the simulation are copied by the simulation software into a own directory.
Once the last simulation finishes the framework aggregates all copied CSV files into single CSV files for each log line type.
Subsequently the \textit{R Markdown} file, which also is used to create the final report of single  simulation, is executed to create a report comparing all simulation runs.
...
Additionally the simulation framework reads the \textit{nodes.csv}, which needs to be available \todo{check with julia}, to determine all planned nodes and their computational share (\textit{share}).
Afterwards the framework parametrizes for each node an exponential distribution as shown in \ref{eq:exponential} with $\lambda = blocks\_per\_tick \cdot share$.
...
This is repeated for every tick until reaching \textit{amount\_of\_ticks}.
For example if we calculated the 5 samples 0.4, 0.8, 2.3, 4.1 and 5.8 for an arbitrary node A.
Furthermore the desired \textit{amount\_of\_ticks} would be 5.
Then we would get 5 ticks, where in the 1st tick are two block events for node A, and in the 3rd tick  and 5th tick one each \todo{check with julia}.
...
On execution of the \textit{simulate} command the simulation framework starts a simulation based on the configuration files \textit{nodes.csv}, \textit{network.csv} and \textit{tick.csv}.
...
\begin{figure}[t]
\includegraphics[width=10cm]{simulation_sequence}
\centering
\caption{Flow chart of the \textit{multi-run} command}
