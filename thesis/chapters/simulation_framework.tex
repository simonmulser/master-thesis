...
\aju{Better avoid unspecified terms like ''small'' in scientific writing}
...
\begin{itemize}
  ...
  \aju{Does this mean a node A with latency 100 and a node B with latency 100 will not have latency 200 for their communication but 100?}
...
\aju{This paragraph reads a bit differently than the tick description. I tried formulating it a bit differently by removing a sentence.}
...
\aju{Explicitly state that this happens on the host to avoid confusion}
...
\aju{Just as a note - it would be more efficient to other way round i.e., starting at the highest block height and then go backwards ;)}
...
\end{itemize}
\aju{Generally I guess the thesis would profit from a glossary describing certain terms like: Tip, consensus chain, compact block,  ... But this is a lot of work. Although you should describe what you mean by tip when using it first. }
...
\aju{The patch might be valuable as diff in the appendix - to show that the change is minimal}
...
\begin{itemize}
    ...
    \aju{What is a planned block event in that context?}
...
\aju{This part could be more detailed, it describes the core property of how the simulation is set up - especially the example is difficult to get. Better illustrate how this expected value - which is later the input value $\beta$ to the \emph{np.random.exponential()} function is constructed i.e., Miner with $ 33\% $ of the hash rate, has an expected value of $ 30 $ min. of finding a block. A block is separated into lets say 20 ticks. So $ \beta = 20 * 30 $. 
Also a reference to papers that justify a exponential distribution here would be good e.g., the classical Bitcoin paper von Nakamoto and the paper of Meni Rosenfeld. There has also been a recent paper that suggest a slightly different distribution \url{https://arxiv.org/pdf/1801.07447.pdf} }
...
\aju{Here a brief section about the storage during the simulation i.e., the ramfs would also be nice, maybe rename this seciton to ''Storage of simulation data'' and have a brief subsection on during the simulation and a long-term subsection.}
...
\aju{Also for this code snippet a diff file in the Appendix would be nice}
