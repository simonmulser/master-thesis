The simulation framework provides all needed functionalities to orchestrate a peer-to-peer network where the nodes are running the \textit{Bitcoin} reference implementation.
...
Furthermore the usage of ticks would allow to speed up the simulation by cutting out empty ticks or ticks which do not influence the investigated results of the simulation.
...
If such a block event is over, all ticks afterwards until the next tick with a block event could be ignored because those ticks would not influence block related simulation results.
...
In the case that the host system is underprovisoned and does not provide enough resources to the nodes the simulation results may be distorted.
...
\textit{Docker} on the other side provides the needed capabilities to run the \textit{Bitcoin} reference implementation and other programs in lightweight containers on one single host as explained in chapter \ref{chap:virtualisation}.
...
If the node now persists a block, the block occupies a power of two big space in the used \textit{blk-file}. Then afterwards the \textit{ZFS} file system is able to deduplicate the aligned block on the files system.
