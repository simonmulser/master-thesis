...
\cite{nayak2016stubborn} provided a comprehensive description of the strategy space and also coined different names for the selfish mining variations:
...
\cite{eyal2014majority, sapirshtein2016optimal} used a closed-source simulations which we cannot discuss here.
...
They additionally used in their MDP the so-called stale block rate which denotes the ratio between blocks from the consensus chain and stale blocks.
...
The higher the stale block rate, the less secure the blockchain protocol is \cite{gervais2016security}.
...
This means whenever a node receives two blocks of the same height it randomly select on of the blocks to mine on.
