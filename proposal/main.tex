...
\todo{This sentence is somehow misleading. As fas as I remember the 50\% thing only becomes an issue when a miner invests is additional relative earnings (in relation to other miners) into new mining hardware and then at some point reach 50\% with this investment. So this is the big-picture long-term-attack scenario not the short term one. By leaving out two sentences this inconsitency resolves it self I guess ...}
...
\todo{Doesnt gervais2016security provide ns3 code for a DES?}
...
\todo{You are referring to the simulation of classical mining behaviour here?}
\end{itemize}

In this thesis we
\todo{It is common to use \emph{we} to refer to the authors even if there is just one ;)}
...
%for any \emph{Bitcoin-like} protocol (e.g., Bitcoin-core or Bitcoin Unlimited) by only adapting the log parsing and RPC logic.
%% Maybe we should emphasise this last point as well somewhere? 
...
\todo{Since this was not yet the focus of previous simulation ... as far as I remember?}
...
\todo{As from your conversation with Eyal - its better to find a reference where it is implemented than to find a reference where its not implemented :)}
