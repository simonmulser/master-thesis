...
Selfish mining was coined in 2014 by \citeauthor{eyal2014majority} in their paper \textit{"Majority is not enough: Bitcoin mining is vulnerable."}. They showed how the dishonest behavior selfish mining can help a miner to earn more than his share of computational power in the network. The profitability of the simple withholding strategy was proved by modeling the selfish mining as state machine and applying probability calculations. Furthermore results of an undisclosed Bitcoin protocol simulator were given. In the simulation 1000 miners with the same simulated mining power were simulated and a fraction of these miners formed a pool which applied the selfish mining algorithm. In the case of to competing blocks they artificially split the network where one part was mining on the block of the public network and one part was mining on the block of the selfish pool. Additionally to the introduction and analysis of the selfish mining they proposed a backward-compatible patch to the Bitcoin reference implementation. In the case of two competing blocks each miner should choose randomly on which block to mine \cite{eyal2014majority}. This proposed change to the Bitcoin protocol was never implemented in the reference implementation of Bitcoin \cite{bitcoin2017}.

Further research showed that more generalized selfish mining strategies lead to even more profitability for the selfish miner \cite{sapirshtein2016optimal, nayak2016stubborn}. \citeauthor{sapirshtein2016optimal} used MDPs to calculate optimal mining strategies for different shares of computation power and network connectivities. Additionally he mentions that selfish mining lowers the costs for the double spending attack and that the attacker has actually no costs for continuously trying to execute a double spending attack. \citeauthor{nayak2016stubborn} on the other hand enumerated the strategy space and used numeric simulations to determine the perfect mining strategy. He additionally combined selfish mining with an eclipse attack. In this case the selfish miner can use the computation power of the eclipsed node to amplify his computation power. The eclipsed node profits from the eclipse and has no incentive to stop the attack \cite{nayak2016stubborn}. 
